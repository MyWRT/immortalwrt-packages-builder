...
(function () {
  ...
  let resultsPointerGestureActive = false;
  let pendingResultsRefresh = false;
  let pendingResultLink = null;
  let pendingResultRect = null;
  let pendingResultGestureHandled = false;
  let resultOptionSeq = 0;
  let menuItems = [];
  let isIndexed = false;
  ...
  let lastPrefetchPersistenceError = "";
  let lastPrefetchCacheLoadError = "";
  let prefetchCollectorWarnings = [];
  ...
  const RESULT_GESTURE_SLACK_PX = 6;
  const RESULT_OPTION_ID_PREFIX = "proton-search-option-";
  ...
  const PREFETCH_RENDER_SETTLE_DELAY = 800;
  const PREFETCH_ROUTE_DELAY = 320;
  const PREFETCH_ERROR_DETAILS_LIMIT = 24;
  ...
  function recordPrefetchDetail(detail) {
    ...
    recentErrors.push(normalizedDetail);
  ...
  function recordPrefetchError(route, error) {
    recordPrefetchDetail(
      `${normalizePrefetchErrorRoute(route)}: ${normalizePrefetchErrorMessage(error)}`,
    );
  }
  ...
  function applyLoadedPrefetchCache(rawCache, rawMeta) {
    ...
    try {
      ...
      if (!cache || !Array.isArray(cache.entries)) {
        lastPrefetchCacheLoadError =
          "cached payload has no entries array (corrupted cache)";
        return "invalid";
      }
      if (Number(cache.version) !== SEARCH_CACHE_VERSION) {
        lastPrefetchCacheLoadError = `cache version ${cache.version} does not match expected ${SEARCH_CACHE_VERSION}`;
        return "outdated";
      }
      ...
      if (!normalizedEntries.length && cache.entries.length) {
        lastPrefetchCacheLoadError = `all ${cache.entries.length} cached entries failed normalization`;
        return "invalid";
      }
      ...
      lastPrefetchCacheLoadError = normalizePrefetchErrorMessage(error);
      return "invalid";
    }
  }

  function describeRpcStatus(status) {
    if (status === UBUS_STATUS_OK) return "";
    if (!Number.isFinite(status)) return "malformed ubus response";
    ...
    return knownStatuses[status] || `ubus status ${status}`;
  }
  ...
  async function callSearchCacheRpc(method, payload) {
    ...
    try {
      ...
      const status = Array.isArray(result?.result)
        ? Number(result.result[0] || 0)
        : Number.NaN;
      const data = extractUbusPayload(result);
      let error = "";

      if (result && result.error) {
        error = `${method}: ${normalizePrefetchErrorMessage(
          result.error.message || result.error,
        )}`;
      } else if (status !== UBUS_STATUS_OK) {
        error = `${method}: ${describeRpcStatus(status)}`;
      } else if (data && typeof data.error === "string" && data.error) {
        error = `${method}: ${data.error}`;
      }
      ...
      if (error && error.name === "AbortError") {
        ...
        return {
          status: Number.NaN,
          data: null,
          error: `${method}: request timed out after ${SEARCH_CACHE_RPC_TIMEOUT} ms`,
        };
      }

      console.warn("[Proton2025] Search cache RPC failed:", method, error);
      return {
        status: Number.NaN,
        data: null,
        error: `${method}: ${normalizePrefetchErrorMessage(error)}`,
      };
    } finally {
      if (timeoutId) {
  ...
  async function readRemotePrefetchCache() {
    ...
    const rawMeta =
      ...
      typeof response.data?.meta === "string"
        ...
        : "";
    const success =
      response.status === UBUS_STATUS_OK && !!response.data?.success;

    return {
      status: response.status,
      success: success,
      error: success ? "" : response.error || "loadCache: unexpected response",
      rawCache: rawCache,
      rawMeta: rawMeta,
      cacheBytes:
  ...
  async function writeRemotePrefetchCache(rawCache, rawMeta) {
    ...
    return {
      status: response.status,
      success: success,
      error: success ? "" : response.error || "saveCache: unexpected response",
      cacheBytes:
        Number(response.data?.cacheBytes) || rawCache.length + rawMeta.length,
    };
  }
  ...
  async function clearRemotePrefetchCache() {
    ...
    return {
      status: response.status,
      success: success,
      error: success ? "" : response.error || "clearCache: unexpected response",
    };
  }
  ...
  async function savePrefetchCache(routeHash, storedAt) {
    ...
    lastPrefetchPersistenceError = remoteResult.error || "";
    return false;
  }
  ...
  async function loadPrefetchCache() {
    ...
    prefetchCacheLoadPromise = (async () => {
      ...
      if (!remoteCache.success) {
        lastPrefetchCacheLoadError = remoteCache.error || "";
      }
      ...
      if (outcome === "unavailable" || outcome === "invalid") {
        recordPrefetchError(
          ...
          lastPrefetchCacheLoadError ||
            (outcome === "unavailable"
              ? "Router did not return a stored search index."
              : "Stored search index is unreadable."),
        );
      } else if (outcome === "outdated") {
        recordPrefetchError(
          "search cache load",
          lastPrefetchCacheLoadError ||
            "Stored search index was built by an older theme version and was discarded.",
        );
      }

      if (outcome === "outdated") {
        const cleared = await clearRemotePrefetchCache();
        if (!cleared.success) {
          recordPrefetchError("search cache reset", cleared.error);
        }
  ...
  function collectPrefetchSafely(label, route, collect) {
    try {
      collect();
      return true;
    } catch (error) {
      prefetchCollectorWarnings.push(
        `${normalizePrefetchErrorRoute(route)} [${label}]: ${normalizePrefetchErrorMessage(error)}`,
      );
      console.warn(
        "[Proton2025] Search index collector failed:",
        label,
        route,
        error,
      );
  ...
  function drainPrefetchCollectorWarnings() {
    const warnings = prefetchCollectorWarnings;
    prefetchCollectorWarnings = [];
    return warnings;
  }
  ...
  function collectDocumentStructuredEntries(
    ...
    sources.forEach((source) => {
      collectPrefetchSafely(source.label, baseHref, () => {
        const nodes = sourceDocument.querySelectorAll(source.selector);
        let failedNodes = 0;
        ...
        nodes.forEach((node) => {
          try {
            ...
            failedNodes += 1;
            lastNodeError = nodeError;
          }
        });

        if (failedNodes > 0) {
          throw new Error(
            `${failedNodes} of ${nodes.length} nodes failed: ${normalizePrefetchErrorMessage(lastNodeError)}`,
          );
        }
      });
    });
  ...
  function collectDocumentCustomTitleEntries(
    sourceDocument,
    baseHref,
    prefetchedItems,
    prefetchedByHref,
    ...
    const pageTitle = getDocumentPageTitle(sourceDocument);
    const candidates = sourceDocument.querySelectorAll(
      "#view [class], #maincontent [class], #view [data-search-title], #maincontent [data-search-title]",
    );
    ...
    candidates.forEach((node) => {
      ...
      const isNamedByClass = /(?:^|[-_])(?:title|name)$/i.test(classNames);
      const isExplicitlySearchable = node.hasAttribute("data-search-title");
      if (!isNamedByClass && !isExplicitlySearchable) return;

      const text = isExplicitlySearchable
        ? node.getAttribute("data-search-title") || node.textContent
        : node.textContent;
      const normalizedText = String(text || "").trim();
      if (!shouldIndexPageText(normalizedText)) return;
  ...
  function extractPrefetchedPageEntries(sourceDocument, routeHref) {
    ...
    collectPrefetchSafely("page title", baseHref, () => {
      const pageTitle = getDocumentPageTitle(sourceDocument);
    ...
    collectPrefetchSafely("tab panes", baseHref, () => {
    ...
    collectPrefetchSafely("tab links", baseHref, () => {
    ...
    collectPrefetchSafely("custom titles", baseHref, () => {
      collectDocumentCustomTitleEntries(
        sourceDocument,
        baseHref,
        prefetchedItems,
        prefetchedByHref,
  ...
  function isPrefetchDocumentReady(sourceDocument) {
    ...
    const view =
      sourceDocument.querySelector("#view") ||
      sourceDocument.querySelector("#maincontent");
    ...
    return new Promise((resolve) => {
      ...
      function check() {
        ...
        if (isPrefetchDocumentReady(sourceDocument)) {
          window.setTimeout(() => {
            resolve(frame.contentDocument || sourceDocument);
          }, PREFETCH_RENDER_SETTLE_DELAY);
          return;
        }
      ...
      frame.onload = async () => {
        try {
          ...
          try {
            ...
            const collectedKeys = Object.create(null);
            const collectCurrentState = () => {
              extractPrefetchedPageEntries(sourceDocument, href).forEach(
                (entry) => {
                  const key = `${entry.href}\n${entry.text}`;
                  if (collectedKeys[key]) return;
            ...
            const tabLinks = Array.from(
              sourceDocument.querySelectorAll(
                ".cbi-tabmenu a[href], #tabmenu a[href]",
              ),
            );
            ...
            for (const tabLink of tabLinks) {
              tabLink.click();
              await new Promise((resolve) => {
                window.setTimeout(resolve, PREFETCH_RENDER_SETTLE_DELAY);
              });
              collectCurrentState();
            }
            ...
            prefetchCollectorWarnings.push(
              `${normalizePrefetchErrorRoute(href)} [page parse]: ${normalizePrefetchErrorMessage(extractError)}`,
            );
            console.warn(
              "[Proton2025] Failed to extract entries for route:",
              href,
              extractError,
            );
            entries = [];
  ...
  async function ensurePrefetchedPagesIndexed(forceRefresh) {
    ...
    preindexLoading = (async () => {
      ...
      if (isPrefetchDocumentReady(document)) {
        extractPrefetchedPageEntries(document, currentPageHref).forEach(
          (entry) => nextPrefetchedEntries.push(entry),
        );
      }
      ...
      prefetchStatus.recentErrors = [];
      prefetchStatus.indexedPages = [];
      drainPrefetchCollectorWarnings();
      lastPrefetchPersistenceError = "";
      dispatchSearchIndexState();
      ...
      for (let index = 0; index < routes.length; index++) {
        ...
        try {
          ...
          drainPrefetchCollectorWarnings().forEach((warning) => {
            prefetchStatus.errorCount += 1;
            recordPrefetchDetail(warning);
          });
          dispatchSearchIndexState();
        }
      ...
      if (isPrefetchDocumentReady(document)) {
        const currentEntries = extractPrefetchedPageEntries(
          document,
          currentPageHref,
        );
        const existingKeys = Object.create(null);
        nextPrefetchedEntries.forEach((entry) => {
          existingKeys[`${entry.href}\n${entry.text}`] = true;
        });
        currentEntries.forEach((entry) => {
          const key = `${entry.href}\n${entry.text}`;
          if (!existingKeys[key]) {
            existingKeys[key] = true;
            nextPrefetchedEntries.push(entry);
          }
        });
      }
      ...
      const shouldReplaceCache =
        nextPrefetchedEntries.length > 0 ||
      ...
      if (shouldReplaceCache) {
        ...
        if (routes.length > 0) {
          ...
          recordPrefetchError(
            "search index",
            `No searchable content collected from ${routes.length} routes; previous cache kept.`,
          );
        }
        dispatchSearchIndexState();
      }
  ...
  const normalizedTitleIncludesCache =
    typeof WeakMap === "function" ? new WeakMap() : null;

  function getNormalizedTitleIncludes(entry) {
    const source = Array.isArray(entry.titleIncludes)
      ? entry.titleIncludes
      : [];

    if (!normalizedTitleIncludesCache) {
      return source.map(normalizeText);
    }

    let cached = normalizedTitleIncludesCache.get(entry);
    if (!cached) {
      cached = source.map(normalizeText);
      normalizedTitleIncludesCache.set(entry, cached);
    }
  ...
  function matchesSemanticEntry(item, entry) {
    const hrefIncludes = Array.isArray(entry.hrefIncludes)
      ? entry.hrefIncludes
      : [];
    const titleIncludes = getNormalizedTitleIncludes(entry);
    ...
    if (
      titleIncludes.some((fragment) => item.titleBlob.indexOf(fragment) !== -1)
    ) {
      return true;
    }
  ...
  const FOCUS_REINDEX_MIN_INTERVAL_MS = 3000;
  let lastForcedIndexTime = 0;

  function shouldForceReindexOnFocus() {
    const now = Date.now();
    if (
      !isIndexed ||
      now - lastForcedIndexTime > FOCUS_REINDEX_MIN_INTERVAL_MS
    ) {
      lastForcedIndexTime = now;
      return true;
    }
    return false;
  }
  ...
  function clearPendingResultGesture() {
    pendingResultLink = null;
    pendingResultRect = null;
    pendingResultGestureHandled = false;
    resultsPointerGestureActive = false;
  }

  function isPointerWithinPendingResult(event) {
    if (!pendingResultRect) return true;
    if (
    ...
    const slack = RESULT_GESTURE_SLACK_PX;
    return (
      event.clientX >= pendingResultRect.left - slack &&
      event.clientX <= pendingResultRect.right + slack &&
      event.clientY >= pendingResultRect.top - slack &&
      event.clientY <= pendingResultRect.bottom + slack
    );
  }

  function finishPendingResultGesture(event) {
    const link = pendingResultLink;
    if (!link) {
      clearPendingResultGesture();
      return;
    }

    const withinBounds = isPointerWithinPendingResult(event);
    clearPendingResultGesture();
    pendingResultGestureHandled = true;
  ...
  function renderResults(matches, queryVariants) {
    ...
    for (let i = 0; i < maxResults; i++) {
      ...
      link.id = `${RESULT_OPTION_ID_PREFIX}${++resultOptionSeq}`;
      ...
      link.addEventListener("pointerdown", (event) => {
        ...
        event.preventDefault();
        resultsPointerGestureActive = true;
        pendingResultLink = link;
        pendingResultGestureHandled = false;
      ...
      link.addEventListener("click", (event) => {
        ...
        event.preventDefault();
        if (pendingResultGestureHandled) {
          pendingResultGestureHandled = false;
          return;
        }
  ...
  function initSearch() {
    ...
    searchInput.setAttribute("aria-label", getSearchPlaceholder());
    ...
    document.addEventListener("pointerup", (event) => {
      if (!resultsPointerGestureActive) return;

      finishPendingResultGesture(event);
    });
  ...
  function onFocus() {
    clearSearchCloseAnimation();
    indexMenu(shouldForceReindexOnFocus());
  ...
  function onInput() {
    if (resultsPointerGestureActive) {
  ...
  function hideResults() {
    ...
    const wasExpanded = searchWrapper.classList.contains("active");
    ...
    // Без проверки раскрытия клик в любом месте страницы запускал бы
    // анимацию свёртывания и на 450 мс прятал меню в шапке.
    if (wasExpanded && searchInput && document.activeElement !== searchInput) {
      startSearchCloseAnimation();
    }
  }

  function mergeLivePageSearchEntries() {
    if (!isPrefetchDocumentReady(document)) return;
    ...
    currentEntries.forEach((entry) => {
      ...
      knownKeys[key] = true;
      prefetchedPageEntries.push(entry);
      added = true;
    });

    if (added) {
      dispatchSearchIndexState();
      if (searchInput && normalizeText(searchInput.value)) onInput();
    }
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("click", () => {
      window.setTimeout(
        mergeLivePageSearchEntries,
        PREFETCH_RENDER_SETTLE_DELAY,
      );
    });
    const liveContent = document.getElementById("view");
    if (liveContent) {
      let liveMergeTimer = 0;
      const liveObserver = new MutationObserver(() => {
        window.clearTimeout(liveMergeTimer);
        liveMergeTimer = window.setTimeout(
          mergeLivePageSearchEntries,
          PREFETCH_RENDER_SETTLE_DELAY,
        );
      });
      liveObserver.observe(liveContent, { childList: true, subtree: true });
    }
    if (typeof window.requestIdleCallback === "function") {
      window.requestIdleCallback(() => indexMenu(true), { timeout: 1000 });
    } else {
      setTimeout(() => indexMenu(true), 0);
    }
